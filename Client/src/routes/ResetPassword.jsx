...
const ResetPassword = () => {
    ...
    const validateUser = async () => {
        const res = await fetch(`${process.env.REACT_APP_BACKEND_URL}/api/v1/auth/reset-password/${id}/${token}`, {
            method: 'GET',
            headers: {
    ...
    const handle = async (e) => {
        ...
        if(validation){
            e.preventDefault();
            const response = await fetch(`${process.env.REACT_APP_BACKEND_URL}/api/v1/auth/reset-password/${id}/${token}`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
